...
from tensorflow.keras.layers import Conv2D ,GRU, LSTM,Input ,MaxPool2D
...
DATA = "E:\PycharmProjects\ML\Dataset"
...
X_train = tf.reshape(X_train,(700,-1,IMG_SIZE,IMG_SIZE,1))
X_test = tf.reshape(X_test,(300,-1,IMG_SIZE,IMG_SIZE,1))
...
for dense_layer in dense_layers:
    for layer_size in layer_sizes:
        ...
        for l in range(lstm_layer-1):
            lstm = Sequential()
            lstm.add(GRU(layer_size, return_sequences=True))
        ...
        for l in dense_layers:
            ...
            dense.add(Dense(1))
            dense.add(Activation('sigmoid'))
            tboard_log_dir = os.path.join("../../CNN_LSTM/10LOGS/CNN-GRU/25-04-21"
                                          , NAME)
